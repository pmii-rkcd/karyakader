...
// PERBAIKAN SAKTI: Membersihkan HTML dan kode entitas agar teks rapi
const stripHtml = (htmlString: string) => {
  if (!htmlString) return '';
  // 1. Ganti tag block menjadi spasi
  let text = htmlString.replace(/<\/?[^>]+(>|$)/g, " ");
  // 2. Bereskan simbol-simbol HTML
  text = text.replace(/&nbsp;/g, ' ')
             .replace(/&quot;/g, '"')
             .replace(/&amp;/g, '&')
             .replace(/&#39;/g, "'")
...
export default function KabarPage() {
  ...
  const BATCH_SIZE = 6; // Menggunakan kelipatan 2 agar grid rapi
              ...
              {articles.map((article) => (
                <div key={article.id} className="bg-white dark:bg-[#0d1520] rounded-2xl border border-gray-100 dark:border-gray-800/60 overflow-hidden hover:shadow-2xl transition-all duration-500 group flex flex-col h-full">
                  ...
                  <div className="p-6 flex flex-col flex-1 min-w-0">
                    ...
                    {/* PERBAIKAN: line-clamp-3 untuk membatasi deskripsi dan stripHtml yang resik */}
