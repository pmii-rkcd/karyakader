...
export default function RedaksiPage() {
  ...
  // Mengambil daftar redaksi dari Firebase
  ...
  // Menyimpan Anggota Redaksi Baru
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setName(''); setRole(''); setImage(null);
      fetchMembers(); // Refresh daftar
    } catch (error) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {members.map((member) => (
                <div key={member.id} className="flex items-center gap-4 p-3 border border-gray-100 rounded-lg shadow-sm hover:shadow-md transition bg-white group relative">
                  <div className="w-16 h-16 relative rounded-full overflow-hidden shrink-0 border-2 border-yellow-500">
